# EBI-Metagenomics/envident: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/envident/usage](https://nf-co.re/envident/usage)
...
sample,fastq_1,fastq_2,single_end,forward_primer,reverse_primer
...
```bash
nextflow run EBI-Metagenomics/envident --input ./samplesheet.csv --outdir ./results --genome GRCh37 -profile docker
```
...
```bash
nextflow run EBI-Metagenomics/envident -profile docker -params-file params.yaml
```
...
```bash
nextflow pull EBI-Metagenomics/envident
```
