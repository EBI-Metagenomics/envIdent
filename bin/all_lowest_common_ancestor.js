...
// BOTH the PID and qcov thresholds, compute the LCA of just those hits
// and truncate it to that rank's depth, then stop.
function rankedLca(hits, ranks) {
  for (const rank of ranks) {
    ...
    const common = lca(passing.map(h => h.lineage));
    return common.slice(0, rank.depth);
  }
  return []; // nothing passed even the coarsest thresholds
}
...
async function main(file, pidThresholdArg, qcovThresholdArg) {
  ...
  for (const [id, hits] of groups) {
    const common = rankedLca(hits, ranks);
